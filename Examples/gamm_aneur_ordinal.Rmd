---
title: "aneur: generalized additive mixture model (GAMM) ordinal"
output: 
  pdf_document: 
...
Help for ordinal logistic regresison in  STAN \\
...
k1 = 5
k2 = 5
knots1 = quantile(x1, c(0.25,0.5,0.75))
knots2 = quantile(x2, c(0.25,0.5,0.75))
...
```{R, eval=TRUE, fig.show="hold", echo=TRUE, include=TRUE, out.width="100%", warning=FALSE, message=FALSE} 
fit.lme.add.incr.reslope <- stan("gamm_aneur_ordinal_lme_add_incr_reslope.stan",
...
stan_dens(fit.lme.add.incr.reslope, pars=c("kappa"))
```
...
ynew = summary(fit.lme.add.incr.reslope)$summary[,"50%"]
ynew = ynew[5816:7202]

aneur3$mu1 = mu1[,"mean-all chains"] 
aneur3$mu2 = mu2[,"mean-all chains"] 
aneur3$ynew = ynew 

ggplot(data=aneur3, 
       mapping=aes(x=age,y=mu2,group=ptnum)) + 
  geom_line(color=aneur3$ptnum) +
  theme_bw() +
  xlab("Age at examination in years") + ylab("Linear predictor") + ggtitle("Estimated profiles by patient")  

ggplot(data=aneur3, 
       mapping=aes(x=age,y=ynew,group=ptnum)) + 
  geom_line(color=aneur3$ptnum) +
    theme_bw() +
